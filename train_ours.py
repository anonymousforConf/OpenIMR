...
def parse_argsion():
    ...
    parser.add_argument('--filter', type=int, default=1,
                        help='whether use a threshold to filter pseudo-labeled nodes')
...
def main():
    ...
    for epoch in range(1, args.epochs+1):
        ...
        seen, novel = [], []
        for i in range(round(n_class/2)):
            seen.extend(list(np.where(targets==i)[0]))
        seen_variancs, seen_centers = compute_var_mean(emb[seen], targets[seen])

        for i in range(round(n_class/2), n_class):
            novel.extend(list(np.where(targets==i)[0]))
        novel_variancs, novel_centers = compute_var_mean(emb[novel], targets[novel]-round(n_class/2))
